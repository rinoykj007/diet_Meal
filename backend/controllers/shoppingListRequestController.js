...
exports.createRequest = async (req, res) => {
  try {
    const { mealPlanId, items, deliveryAddress, estimatedCost, notes } =
      req.body;
    ...
    const populatedRequest = await ShoppingListRequest.findById(
      request._id
  ...
  try {
    ...
    if (request.deliveryPartnerId) {
      return res.status(400).json({
        success: false,
        message:
          "This request has already been accepted by another delivery partner",
      });
    }
  ...
  try {
    ...
    const statusMessages = {
      ...
      delivered: {
        ...
        message:
          "Your shopping items have been delivered. Please make payment to the delivery partner.",
      },
    };
